...
public class ScrollerNumberPicker extends View {
		...
		for (ItemObject item : itemList) {
			if (item.isSelected()) {
				if (onSelectListener != null)
					onSelectListener.endSelect(item.id, item.city_id, item.itemText);
				break;
			}
		}
	}
	...
	private void initData() {
		...
		for (int i = 0; i < dataList.size(); i++) {
			...
			itmItemObject.id = i;
			itmItemObject.city_id = dataList.get(i).city_id;
			itmItemObject.itemText = dataList.get(i).cityName;
			itmItemObject.x = 0;
		...
		if (move > 0) {
			for (int i = 0; i < itemList.size(); i++) {
				if (itemList.get(i).isSelected()) {
					...
					if (onSelectListener != null)
						onSelectListener.endSelect(itemList.get(i).id, itemList.get(i).city_id, itemList.get(i).itemText);
					break;
				}
			}
		} else {
	...
	private void onSelectListener() {
		...
		for (ItemObject item : itemList) {
			if (item.isSelected()) {
				onSelectListener.selecting(item.id, item.city_id, item.itemText);
			}
		}
	}
	...
	private void drawLine(Canvas canvas) {
		...
		// float startX = 0;
		// float startY = controlHeight / 2 - unitHeight / 2 + 2;
		// float stopX = controlWidth;
		// float stopY = controlHeight / 2 - unitHeight / 2 + 2;
	...
	public void getHitRect(Rect outRect) {
		...
		outRect.set(outRect.left, controlHeight / 2 - unitHeight / 2, outRect.right, controlHeight / 2 + unitHeight / 2);
	}

	public void getViewRect(Rect outRect) {
		super.getHitRect(outRect);
	}
	 ...
	 * @return
	 */
	public String setData(List<CityInfo> data) {
		this.dataList = data;
		initData();
		if (!dataList.isEmpty()) {
			setDefault(0);
			return dataList.get(0).city_id;
		}
		return "";
	}
